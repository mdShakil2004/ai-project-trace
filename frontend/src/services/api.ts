...
type InvestigationRealtimeHandlers = {
  onEvents: (events: AnalysisEvent[]) => void;
  onStage?: (event: AnalysisEvent) => void;
  onComplete: () => void;
...
export const api = {
  ...
  subscribeInvestigation: (
    id: string,
    realtimeToken: string,
    handlers: InvestigationRealtimeHandlers,
  ) => {
    const socket = new WebSocket(
      `${websocketBase()}/ws/investigations/${encodeURIComponent(id)}?token=${encodeURIComponent(realtimeToken)}`,
    );
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (payload.type === 'snapshot') {
          handlers.onEvents(payload.events || []);
        } else if (payload.type === 'stage') {
          const stage = payload.event as AnalysisEvent | undefined;
          if (stage?.id) handlers.onStage?.(stage);
        } else if (payload.type === 'completed') {
          handlers.onComplete();
        } else if (payload.type === 'failed') {
          handlers.onError(payload.message || 'Analysis failed.');
        } else if (payload.type === 'error') {
          handlers.onError(payload.message || 'Realtime progress is unavailable.');
        }
      } catch {
    ...
    socket.onclose = () => {
      if (!closedByCaller) handlers.onError('Realtime connection closed.');
    };
  ...
  analyzePullRequest: async (url: string) => {
    const result = await request<InvestigationStart>('/investigations', {
      method: 'POST',
      body: JSON.stringify({ pull_request_url: url }),
    });
    return {
  ...
  askInvestigationQuestion: (id: string, question: string) =>
    request<QuestionAnswer>(`/investigations/${id}/questions`, {
      method: 'POST',
      body: JSON.stringify({ question }),
    }),
  getSettings: () => request<TraceSettings>('/settings'),
  updateSettings: (
    settings: Pick<TraceSettings, 'analysisDepth' | 'riskThreshold' | 'notifyHighRisk'>,
  ) => request<TraceSettings>('/settings', { method: 'PUT', body: JSON.stringify(settings) }),
  ...
  addRepository: (url: string) =>
    request<Repository>('/repositories', { method: 'POST', body: JSON.stringify({ url }) }),
  listNotifications: () => request<TraceNotification[]>('/notifications'),
  markNotificationRead: (id: string) =>
    request<{ id: string; read_at: string }>(`/notifications/${id}/read`, { method: 'PATCH' }),
};
