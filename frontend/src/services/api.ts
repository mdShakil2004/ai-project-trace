...
console.log('API_BASE....',API_BASE,"import.meta.env.VITE_TRACE_API_BASE",import.meta.env.VITE_TRACE_API_BASE);





function websocketBase(){const url=new URL(API_BASE,window.location.origin);url.protocol=url.protocol==='https:'?'wss:':'ws:';url.pathname=url.pathname.replace(/\/api\/v1\/?$/,'');return url.toString().replace(/\/$/,'');}
async function request<T>(path:string,init:RequestInit={}):Promise<T>{const res=await fetch(`${API_BASE}${path}`,{...init,credentials:'include',headers:{'content-type':'application/json',...(init.headers||{})}});const payload=await res.json().catch(()=>null);if(!res.ok)throw new Error(payload?.error?.message??`Request failed [${res.status}]`);return(payload?.data??payload) as T;}
...
export const api={
 ...
 subscribeInvestigation:(id:string,realtimeToken:string,handlers:{onEvents:(events:AnalysisEvent[])=>void;onComplete:()=>void;onError:(message:string)=>void})=>{const socket=new WebSocket(`${websocketBase()}/ws/investigations/${encodeURIComponent(id)}?token=${encodeURIComponent(realtimeToken)}`);let closedByCaller=false;socket.onmessage=event=>{try{const payload=JSON.parse(event.data as string);if(payload.type==='snapshot')handlers.onEvents(payload.events||[]);else if(payload.type==='stage')api.getEvents(id).then(handlers.onEvents).catch(()=>undefined);else if(payload.type==='completed')handlers.onComplete();else if(payload.type==='failed')handlers.onError(payload.message||'Analysis failed.');}catch{handlers.onError('Received an invalid realtime event.');}};socket.onerror=()=>{if(!closedByCaller)handlers.onError('Realtime connection unavailable.');};return()=>{closedByCaller=true;socket.close();};},
 analyzePullRequest:async(url:string)=>{const result=await request<InvestigationStart>('/investigations',{method:'POST',body:JSON.stringify({pull_request_url:url})});return{investigationId:result.id,status:result.status,realtimeToken:result.realtimeToken};},
